<?php 
    ...
    $res = $bdd->query("SELECT * FROM alizon.Client WHERE codeCompte = '1'")->fetch();
    print_r($res);
...
<body>   
    <main>
        ...
        <form action="enreg.php" method="post" enctype="multipart/form-data">
            ...
            <input type="text" name="pseudo" id="identifiant" pattern="[A-Za-z._]{2,20}" value="<?php echo $res["pseudo"]?>" required disabled/> 
            <span>L'identifiant doit faire entre 2 et 20 caractères (lettres, ".", "_" acceptés)</span>
            <div id="nomPrenomCli">
                <div class="labelInput">
                    <label for="nom">Nom *</label>
                    <input type="text" name="nom" id="nomCli" value="<?php echo $res["nom"] ?>" required disabled/>
                </div>
                <div class="labelInput">
                    <label for="prenom">Prénom *</label>
                    <input type="text" name="prenom" id="prenomCli" value="<?php echo $res["prenom"]?>" required disabled/>
                </div>
            </div>
            ...
            <input type="text" name="numTel" id="numTelCli" pattern="[0-9]{10}" value="<?php echo $res["numtel"]?>"disabled/>
            <span>Le numéro doit être dans le format suivant : 0102030405</span>
            <label for="dateNaiss">Date de naissance *</label>
            <input type="date" name="dateNaiss" class="boutonSec" id="dateNaiss" onChange="verifDate(event)" value="<?php echo $res["datenaiss"]?>"required disabled/>
            ...
            <div class="container-fluid p-0">
                <div class="row ">                      
                    <div class="col-3 labelInput">
                        ...
                        <input type="text" name="numRue" id="numRueCli" disabled/>
                    </div>
                    <div class="col-9 labelInput">
                        <label for="nomRue">Nom de la rue, voie</label>
                        <input type="text" name="nomRue" id="nomRueCli" disabled/>
                    </div>
                </div>
                <div class="row ">
                    <div class="col-4 labelInput">
                        <label for="codePostal">Code postal</label>
                        <input type="text" name="codePostal" id="codePostalCli" pattern="^(?:0[1-9]|[1-8]\d|9[0-8])\d{3}$" disabled/>
                    </div>
                    <div class="col-8 labelInput">
                        <label for="ville">Ville</label>
                        <input type="text" name="ville" id="villeCli" disabled/>
                    </div>
                </div>
            </div>
            <label for="numApp">Numéro d'appartement</label>
            <input type="text" name="numApt" id="numAptCli" disabled/>
            <label for="comp">Complément</label>
            <input type="text" name="comp" id="compAdrCli" disabled/>
