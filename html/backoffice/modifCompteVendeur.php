<?php
    ...
    if(isset($_GET["traitement"])){
        switch($_GET["traitement"]){
            case "bloquer":
                    $stmt = $bdd->prepare("UPDATE alizon.Vendeur SET cmtBlq = TRUE WHERE codeCompte = '".$_SESSION["codeCompte"]."'");
                    $stmt->execute();
                    session_destroy();
            case "deconnecter":
                session_destroy();
    ...
    if(isset($_GET["modif"])){
        if( $_GET["modif"] == "mdp"){
            $stmt = $bdd->prepare("UPDATE alizon.Vendeur SET mdp = '".$_SESSION["nouveauMdp"]."' WHERE codeCompte = '".$_SESSION["codeCompte"]."'");
            $stmt->execute();
            $_SESSION["nouveauMdp"] = "";
        }
