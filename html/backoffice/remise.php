...
<body>
    ...
    <div class="right-content"> 
        <?php
        $stmt = $bdd->prepare('SELECT * FROM alizon.FaireReduction JOIN alizon.Reduction ON alizon.FaireReduction.idReduction = alizon.Reduction.idReduction JOIN alizon.Produit ON alizon.FaireReduction.codeProduit = alizon.Produit.codeProduit WHERE alizon.Produit.codeCompteVendeur = \'' . $codeCompte . '\'');
        $stmt->execute();
        $liste_reduc = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ?>
        <div class="mes-produits">
            <h1>Mes produits en réduction</h1>
                <?php 
                $rows = $liste_reduc;
                if (count($rows) != 0) {
                    ...
                    <div class="tous-les-produits">
                        <div class="produit produit-remise">
                            <?php
                            echo '<a href="./ficheProduit.php?Produit='.htmlspecialchars($row['codeproduit']).'">';
                                ...
                                echo '<div class="infoProduit">';
                                    echo '<p class="prixNormal">'.$row['prixht'].'€</p>';
                                    echo '<p class="prixReduc">'.round($row['prixht'] *(1 - $row['remise']/100), 2).'€ <span class="remise"> - '.$row['remise'].'%</span></p>';
                                ...
                                <?php echo '<p class="infoPromo">En réduction du '.$row['datedebut'].' au '.$row['datefin'].'</p>';?>   
                                <?php echo '<a href="delReduction.php?Produit='.$row['codeproduit'].'&page=manage" class="btnSupprReduc">Supprimer la réduction</a>';?>
                ...
                <?php 
                    ...
                    echo "<p>Vous n'avez pas de produits en réduction pour le moment.</p>";
                } ?>
        </div>
    </main>
