...
$bdd->query('set schema \'alizon\'');
    ...
    if(isset($categorie)){ 
        ...
        $base = "SELECT p.codeProduit, p.libelleProd, p.prixTTC, p.urlPhoto, p.noteMoy, p.descriptionProd
                FROM Produit p
                JOIN Categoriser c ON p.codeProduit = c.codeProduit
                WHERE c.libelleCat = :cat
                AND p.Disponible = true";
    }else {
        $base= 'SELECT codeProduit, libelleProd, prixTTC, urlPhoto, noteMoy, descriptionProd FROM Produit where Disponible = true' ;
        $cat = null;
    }
